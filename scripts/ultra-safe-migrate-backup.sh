...
collect_static_files() {
    ...
    # First verify critical files exist before collectstatic
    if [[ ! -f "static/css/style.css" ]]; then
    ...
    # Try collectstatic with manifest storage first
    log_info "Attempting collectstatic with CompressedManifestStaticFilesStorage..."
    local collectstatic_output
    local collectstatic_exit_code
    
    collectstatic_output=$($python_cmd manage.py collectstatic --noinput --verbosity=2 2>&1)
    collectstatic_exit_code=$?
    
    echo "$collectstatic_output" | tee -a "$LOG_FILE"
    
    if [[ $collectstatic_exit_code -eq 0 ]]; then
        log_info "✅ collectstatic with manifest storage completed successfully"
    else
        log_warning "⚠️  collectstatic with manifest storage failed, trying fallback..."
        
        # Check if it's a manifest-related error
        if echo "$collectstatic_output" | grep -i "manifest\|missing\|couldn't find"; then
            log_info "🔧 Detected manifest storage issue, temporarily switching to basic storage..."
            
            # Temporarily disable manifest storage and try again
            collectstatic_output=$($python_cmd manage.py shell -c "
import os
...
" 2>&1)
        ...
        if [[ $collectstatic_exit_code -ne 0 ]]; then
            log_error "❌ Both manifest and basic collectstatic failed"
            return 1
        else
            log_info "✅ collectstatic with basic storage completed successfully"
        fi
    fi
    
    # Verify critical files were collected
    if [[ -f "${static_root}/css/style.css" ]]; then
        ...
        log_error "❌ Critical CSS file NOT found in static_collected after collectstatic"
        log_error "   Expected: ${static_root}/css/style.css"
        log_error "   Listing static_collected/css contents:"
...
main() {
    ...
    if ! build_css; then
        ...
        log_error "   The application will not render correctly without CSS"
        exit 1
    fi
    
    # Step 4: Static files (mandatory for production)
    if [[ "$SKIP_STATIC" != "true" ]]; then
        ...
        if ! collect_static_files; then
            ...
            log_error "   The application will not serve CSS/JS correctly without static files"
            exit 1
        fi
    else
